...
// ── Password hashing (mirrors employeeService.ts) ────────────────────────────

/** Hash a plaintext password with bcrypt (12 rounds, same as the app). */
...
// Default sign-in password: `Password123` (must satisfy the strength rules).
...
const EMPLOYEES: Array<{
  ...
  { employeeId: "emp_tenant-omantel_omt-001", tenantId: "tenant-omantel", employeeCode: "OMT-001", name: "Ahmed Al Balushi", email: "ahmed.balushi@omantel.om", password: EMPLOYEE_PASSWORD },
  { employeeId: "emp_tenant-omantel_omt-002", tenantId: "tenant-omantel", employeeCode: "OMT-002", name: "Mariam Al Siyabi", email: "mariam.siyabi@omantel.om", password: EMPLOYEE_PASSWORD },
  { employeeId: "emp_tenant-oq_oq-001", tenantId: "tenant-oq", employeeCode: "OQ-001", name: "Said Al Hinai", email: "said.hinai@oq.com", password: EMPLOYEE_PASSWORD },
  { employeeId: "emp_tenant-oq_oq-002", tenantId: "tenant-oq", employeeCode: "OQ-002", name: "Noor Al Zadjali", email: "noor.zadjali@oq.com", password: EMPLOYEE_PASSWORD },
  { employeeId: "emp_tenant-pdo_pdo-001", tenantId: "tenant-pdo", employeeCode: "PDO-001", name: "Fatma Al Riyami", email: "fatma.riyami@pdo.co.om", password: EMPLOYEE_PASSWORD },
  { employeeId: "emp_tenant-pdo_pdo-002", tenantId: "tenant-pdo", employeeCode: "PDO-002", name: "Hamed Al Busaidi", email: "hamed.busaidi@pdo.co.om", password: EMPLOYEE_PASSWORD },
];
...
async function main() {
  ...
  try {
    ...
    console.log(`Seeding employees (password: ${EMPLOYEE_PASSWORD} for all)…`);
    const employeesCol = db.collection("employees");

    for (const e of EMPLOYEES) {
      const passwordHash = hashPassword(e.password);
    ...
    console.log(`\nEmployee portal password: ${EMPLOYEE_PASSWORD} (all employees)`);
  } finally {
    await client.close();
  }
}
