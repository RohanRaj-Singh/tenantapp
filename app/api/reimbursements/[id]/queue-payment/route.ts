...
import { queueForPayment } from "@/src/server/services/reimbursementService";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

/**
 * Authorizes queue-for-payment requests.
 * Only Super Admin can queue an approved claim for payout — requires
 * `x-admin-api-key` header matching ADMIN_API_KEY env var. The Super Admin
 * dashboard proxy passes this key.
 */
...
export async function POST(
  ...
  try {
    ...
    const reimbursement = await queueForPayment(
      claim.tenantId,
      id,
      "super-admin",
      body.notes,
