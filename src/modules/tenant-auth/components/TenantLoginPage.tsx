...
export function TenantLoginPage({
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!identifier.trim()) {
      setError(loginCopy.errors.identifierRequired);
      return;
    }

    if (!password) {
      setError(loginCopy.errors.passwordRequired);
      return;
    }

    if (!canSubmit) {
      setError(loginCopy.errors.dashboardUnavailable);
      return;
    }
    ...
    try {
      ...
      if (!response.ok || !payload?.success) {
        setError(payload?.error ?? loginCopy.errors.signInFailed);
        return;
      }
        ...
        <div className="rounded-3xl border border-black/[0.06] bg-white p-10 shadow-2xl shadow-black/[0.04]">
          <div className="text-center">
            ...
            <h1
              ...
              style={{ color: "var(--tenant-primary, #f58220)" }}
            >
              {tenantName ?? loginCopy.signInFallbackTitle}
            </h1>
            <p className="mt-1.5 text-sm text-slate-500">{loginCopy.subtitle}</p>
          </div>

          {!tenantSlug ? (
            <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
              {loginCopy.unresolvedTenant}{" "}
              <code className="font-mono">{loginCopy.unresolvedTenantCode}</code> query
              parameter.
            </div>
          ) : null}
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-4">
            ...
            >
              ...
              {isSubmitting ? loginCopy.submitting : loginCopy.submit}
            </button>
          </form>
        </div>

        <p className="mt-6 text-center text-xs text-slate-400">{loginCopy.help}</p>
      </div>
    </div>
  );
}
